...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push(function () {
  ...
  function d(c) {
    ...
    if ("path" === l)
      (h = c.style.strokeDasharray),
        (c.style.strokeDasharray = "none"),
        (d = c.getTotalLength() || 0),
        (e = c.getBBox()),
        (c.style.strokeDasharray = h);
      ...
      for (
        f = c.getAttribute("points").split(" "),
          d = 0,
          h = f[0].split(","),
          "polygon" === l &&
            (f.push(f[0]), -1 === f[0].indexOf(",") && f.push(f[1])),
          i = 1;
        i < f.length;
        i++
      )
        (g = f[i].split(",")),
          1 === g.length && (g[1] = f[i++]),
          2 === g.length && ((d += a(h[0], h[1], g[0], g[1]) || 0), (h = g));
    else
      "ellipse" === l &&
        ((j = parseFloat(c.getAttribute("rx"))),
